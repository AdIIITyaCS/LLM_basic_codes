...
# #Every time for a single query if we run the code it becomes hectic due to the memory of previous query skipped. So now we want to achieve the same action as any chatgpt like tool does everytime pass the whole stuff to the model by saving the previous memory here.
...
# # #now we are trying that our code itself stores the history not like manually storing by me
# from google import genai
